...
2. *Modeling*. This will allow me to quantify the relationship between the transmission types and miles per gallon. 
...
I conduct three classification algorithems to learning, decision tree, bagging and random forest. The models statistics comparision is shown in  Table \ref{tab:mod_comparison}. From the comparison, we can find the ideal model is the random forest, with a rather high accuracy rate of 99.92%.
...
```{r, echo=FALSE, tidy=TRUE, results='hide', warning=FALSE}
...
Based on the random forest model, furthermore, I dig into the features to refine the model. Figure \ref{fig:varimp} reveals the top important variables, which is a clue for further features reduction. Therefore, I pick up the top 25 most important variable to conduct a new random forest model. 

```{r, varImp, fig.cap="Variable Importance Plot\\label{fig:varimp}", echo=FALSE, warning=FALSE}
varImp <- importance(modrf)
varImpPlot(modrf, type=1, main="Variable Importance Plot",cex = 0.9)
```

The model reaches a even high accuracy rate of 99.96% and a low Out of Sample(OOB) error estimate of 0.11%. From Figure \ref{fig:oobplot}, we can find that the OOB errors are decreasing as the number of trees increases. 
...
```{r, OOBPlot, fig.cap="Out-of-bag (OOB) Error Estimate\\label{fig:oobplot}", echo=FALSE, warning=FALSE}
